...
class MethodMove extends Method {
	 ...
	 * @see Method
	 */
	MethodMove(final StorageManager storageManager, final Uri uri, final ServeRequest query) {
		super( uri, query );
	...
	protected ReplyAnswer run() {
		...
		if (destination.length() == 0) {
			return Reply.string( "DAV", this.query, "Cannot move without destination!" ).setCode( Reply.CD_UNKNOWN );
		}
		...
		if (!destination.startsWith( check )) {
			return Reply.string( "DAV", this.query, "Cannot move outside the storage!" )
		...
		if (file == null || !file.isExist()) {
			return Reply.string( "DAV", this.query, "Cannot move no file exists!" ).setCode( Reply.CD_CONFLICT );
		}
		if (file.isBinary() && !file.canWrite()) {
			return Reply.string( "DAV", this.query, "File is read-only!" ).setCode( Reply.CD_CONFLICT );
		}
		...
		final int code = this.storageManager.moveResource( this.uri, targetUri, "T".equals( overwrite ), this.depth );
